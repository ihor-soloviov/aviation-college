...
### 🛠️ Наповнюєте сайт контентом, а не пишете код?

**[⬇️ Перейти одразу до інструкції «Робота з адміністрування сайту»](#робота-з-адміністрування-сайту)**
...
- [Скрипти](#скрипти)
- [**Робота з адміністрування сайту**](#робота-з-адміністрування-сайту) 👈
- [Деплой](#деплой)
...
## Робота з адміністрування сайту

> Цей розділ — для контент-редакторів. Не потрібно знати код: увесь контент сайту редагується у візуальній адмінці **PayloadCMS**.

### Як зайти в адмінку

1. Відкрийте **`/admin`** (на проді — `http://204.168.161.68/admin`, локально — <http://localhost:3000/admin>).
2. Введіть email і пароль вашого облікового запису.
3. Перший обліковий запис створюється автоматично при першому відкритті `/admin` на новій базі. Наступних редакторів додає адміністратор у розділі **Користувачі** (див. нижче).

> 🔒 Кожна зміна зберігається одразу в базі й одразу видима на сайті — окремого «Опублікувати» немає. Зберігаючи запис, перевіряйте, що заповнили все правильно.

### Що де редагується

Ліворуч в адмінці — список колекцій. Кожна відповідає за свій блок сайту:

| Колекція | Що це на сайті | Типова дія |
|---|---|---|
| **Новини** | стрічка `/news` та сторінки новин | додати/відредагувати новину |
| **Документи** | PDF-файли на `/documents/:id` | завантажити новий PDF, змінити назву/категорію |
| **Освітні програми** | картки курсів і Part-147 | оновити опис спеціальності, код, форму навчання |
| **Сторінки** | контентні сторінки (page-builder) | зібрати сторінку з блоків |
| **Бібліотеки документів** + **Пункти бібліотеки** | деревоподібні переліки документів | побудувати папки/підпункти зі списком PDF |
| **Медіа** | зображення (для новин, сторінок) | завантажити картинку з alt-текстом |
| **Користувачі** | доступ до самої адмінки | додати/прибрати редактора |

> 🌐 Сайт двомовний (**uk / en**). Угорі форми запису є перемикач локалі — текстові поля зберігаються окремо для кожної мови. Заповнюйте українську, а за потреби перемкніться на English і додайте переклад.

### Додати / відредагувати новину

1. **Новини → Create New** (або відкрийте наявну зі списку).
2. Заповніть:
   - **Заголовок** — обов'язково.
   - **Slug** — частина URL; латиницею, без пробілів (напр. `den-vidkrytykh-dverei`). Має бути унікальним.
   - **Короткий опис** — текст для картки у стрічці й для прев'ю в соцмережах.
   - **Дата публікації** — за нею сортується стрічка.
   - **Головне зображення** — обкладинка картки й сторінки (виберіть із **Медіа** або завантажте нове).
   - **Теги** — за бажанням.
3. **Контент** — тіло новини збирається з блоків (кнопка **Add Block**):
   - **Абзац** — форматований текст;
   - **Заголовок** — підзаголовки всередині статті;
   - **Зображення** / **Галерея** — картинки;
   - **YouTube** — вбудоване відео за посиланням;
   - **Список посилань** — блок із покликаннями на документи/сторінки.
   Блоки можна перетягувати, щоб змінити порядок.
4. **Save**.

### Завантажити документ (PDF)

1. **Документи → Create New**.
2. **Назва** — як документ називатиметься на сайті.
3. **Файл** — перетягніть PDF (зберігається в `data/uploads/payload/documents/`).
4. **Категорії** — оберіть одну або кілька; за ними документ потрапляє у відповідні розділи.
5. **Дата публікації** — обов'язково.
6. **Save**. Документ доступний за `/documents/:id`.

> Поле **Legacy ID** заповнене лише в мігрованих зі старого сайту записах — не чіпайте його вручну; воно забезпечує редиректи зі старих `/article/:id`.

### Освітні програми (курси)

**Освітні програми → Create New**. Ключові поля: **Назва спеціальності**, **Код**, **Рівень освіти** (фаховий молодший бакалавр / бакалавр), **Форми навчання** (денна / заочна), **Напрямок** (IT, енергетика, авіація, телеком, менеджмент). Поле **Порядок сортування** визначає, в якому порядку картки йдуть у списку.

### Бібліотеки документів (дерево з папок)

Для великих переліків PDF (нормативка, розклади тощо) використовуйте пару колекцій:

1. **Бібліотеки документів** — створіть саму бібліотеку (заголовок + slug).
2. **Пункти бібліотеки** — додавайте пункти:
   - позначте **Папка**, якщо це папка з вкладеними пунктами;
   - **Бібліотека** — до якої належить пункт;
   - **Батьківська папка** — щоб покласти пункт усередину іншої папки;
   - **Документ** або **Зовнішня URL** — куди веде кінцевий пункт;
   - **Порядок** — сортування в межах папки.

### Керування редакторами

**Користувачі → Create New** — вкажіть **Ім'я**, email і пароль. Щоб забрати доступ — видаліть користувача. Пароль редактор може змінити у своєму профілі (іконка вгорі праворуч).

### Поради й застереження

- **Slug не міняйте без потреби** — зміна ламає наявні посилання на сторінку/новину.
- **Видалення незворотнє** — видалений запис зникає з сайту одразу. Якщо треба лише сховати — краще зверніться до розробника.
- **Зображення — тільки через Медіа**: спочатку завантажте картинку в **Медіа** з осмисленим **alt-текстом**, тоді підключайте її в новині/сторінці.
- **Резервні копії** робляться автоматично під час деплою (дамп SQLite), але звичайні правки контенту не «відкатуються» самі — будьте уважні при масових змінах.

Технічні деталі колекцій — у `src/collections/*.ts`. Як зняти локальну копію бойових даних для тестів — див. [Підтягнути реальні дані з прода](#підтягнути-реальні-дані-з-прода).
